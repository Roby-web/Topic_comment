...
- Tất cả các vị trí hiển thị ngày tháng trong ứng dụng (bao gồm Nút chọn khoảng thời gian Date Picker, Preview khung chọn ngày, Ngày xuất bản bài viết trong Danh sách & Modal Chi tiết, Tiêu đề phân tích) phải **bổ sung thêm thứ trong tuần (tên viết tắt tiếng Anh, ví dụ: Sun, Mon, Tue...)** hoặc tên Thứ tiếng Việt chuẩn mực theo Rule.
...
## 5. Quy tắc Thiết kế Bố cục & Thẻ Trang Nhân sự (Staff Directory Layout & Cards)

- **Tiêu đề trang**: Dùng tiêu đề ngắn gọn là **"Nhân sự"** (thay vì "Danh bạ Nhân sự").
- **Chiều rộng tối đa (Max-width)**: Đưa container chính về `max-w-[1200px]` căn giữa (`mx-auto`).
- **Bố cục 2 Cột (2-Column Layout)**:
  - **Cột Trái (Left 1/4 max width)**: Chuyên dành lọc theo Ban / Đơn vị.
    - Sắp xếp ưu tiên: **`BBT` và `BOD` luôn nằm ở trên cùng**, các Ban còn lại sắp xếp theo thứ tự Alphabet A-Z (tiếng Việt).
    - Hiển thị nhãn kèm số lượng nhân sự trực thuộc từng Ban.
  - **Cột Phải (Right 3/4 max width)**:
    - Chứa thanh Tìm kiếm tức thời và Bộ lọc Vùng (Tất cả, Hà Nội, TP HCM). Đã bỏ bộ lọc "Đang làm việc".
    - Danh sách thẻ nhân sự hiển thị **2 items trên 1 row** (`grid grid-cols-1 md:grid-cols-2 gap-3.5`).
    - Tất cả nhân sự trong danh sách được sắp xếp theo **thứ tự Alphabet A-Z theo tên gọi**.
- **Nguyên tắc phối màu pastel nhẹ nhàng**: Tuyệt đối tránh sử dụng màu gắt/đậm trên các thẻ và nút bấm.
  - *Khung thẻ*: Nền trắng sạch (`bg-white`), viền xám nhạt (`border-[#e2e8f0]`), góc bo `rounded-[8px]`, đệm `p-3.5`.
  - *Avatar*: Hình tròn pastel xanh dương dịu (`bg-[#eff6ff] text-[#1d4ed8] border-[#bfdbfe]`), chữ cái đầu viết hoa.
  - *Họ tên & Máy lẻ*: Danh xưng (`Chị`/`Anh` xám nhạt), Họ tên in đậm (`font-bold text-[#0f172a]`), số máy lẻ nối tiếp (` - 4547`).
  - *IP Phone*: Nhãn pastel vàng hổ phách (`bg-[#fffbeb] border-[#fde68a] text-[#92400e] font-mono font-bold`).
